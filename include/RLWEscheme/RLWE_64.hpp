...
vector<vector<polynomial>> RGSWct_2048(int n, int64_t q, int k, int var, const polynomial & m, 
...
vector<vector<uint64_t>> RGSWct_2048_ntt(int n, int64_t q, int k, int var, const polynomial & m, 
  const polynomial & s, int64_t b, int logb) {

  //vector<vector<polynomial>> rgswct;
  vector<vector<uint64_t>> rgswct(4*(k/logb), vector<uint64_t>(n));

  //~RLWE(m,2^i)
  int64_t temp2k = b;
  polynomial mi = copy(m);

  vector<uint64_t> temppoly(n);
  ...
  for (int i = 0; i < (k/logb); ++i) {
    ...
    for(int j = 0 ; j < 2; ++j){
      for(int h=1;h<=n;++h){
        if(c1i[j][h]<0){
          temppoly[h-1] = (uint64_t)(c1i[j][h]+q);
        }
        else{
          temppoly[h-1] = (uint64_t)c1i[j][h];
        }
      }
      g_ntt->ComputeForward(temppoly.data(), temppoly.data(), 1, 1);
      rgswct[i*2+j].swap(temppoly);
    }
  }
  ...
  for (int i = 0; i < (k/logb); ++i) {
    ...
    for(int j = 0 ; j < 2; ++j){
      for(int h=1;h<=n;++h){
        if(c2i[j][h]<0){
          temppoly[h-1] = (uint64_t)(c2i[j][h]+q);
        }
        else{
          temppoly[h-1] = (uint64_t)c2i[j][h];
        }
      }
      g_ntt->ComputeForward(temppoly.data(), temppoly.data(), 1, 1);
      rgswct[(2*i)+j+2*(k/logb)].swap(temppoly);
    }
  }
...
extern vector<uint64_t> Extract0_q(int64_t q, int64_t delta, int n, const vector<vector<uint64_t>> & RLWE_ct) {
